export class syngyne {
    ...
    triangleTex(array, texture) {
        const camPts = [];
        for (let i = 0; i < 3; i++) {
            const cp = { ...this.camera.transformPoints(array[i]), uv: array[i].uv, color: array[i].color };
            camPts.push(cp);
        }
    ...
    renderLightSplat(light) {
        ...
        for (let yPixel = bounds.minY; yPixel < bounds.maxY; yPixel++) {
            ...
            for (let xPixel = bounds.minX; xPixel < bounds.maxX; xPixel++, idx++) {
                ...
                if (zCorrect === undefined || zCorrect === Infinity)
                    continue;
...
//# sourceMappingURL=syngyne.js.map