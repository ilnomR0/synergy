export class Collision{
    static #colliderSpots = [];
    static gravity = {
        x:0,
        y:0.25,
        z:0 
    }
    
    static addCollider(mesh){
        this.#colliderSpots.push(mesh); 
    }

    static rigidify(object, dt){
        object.velocity.y += this.gravity.y;
        object.velocity.x += this.gravity.x;
        object.velocity.z += this.gravity.z;
        object.position.y += object.velocity.y*dt;
        object.position.x += object.velocity.x*dt;
        object.position.z += object.velocity.z*dt;

        if(object.position.y >= -1){ //TEMPORARY COLLISION THINGS
            object.position.y -=object.velocity.y*dt;
            object.velocity.y -= this.gravity.y;
            object.velocity.y = 0;
            object.onGround = true;

            object.velocity.x /=1.2;
            object.velocity.z /=1.2;
        }else{
            object.onGround = false;
            object.velocity.x /=1.05;
            object.velocity.z /=1.05;
        }
    }
}
