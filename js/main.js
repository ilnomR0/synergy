...
const syn = new syngyne();
const player = new Character({ syn });
...
syn.addLight(light);
syn.addLight(light2);
Collision.addCollider(wall1, wall2, wall3, wall4, wall5);
let lastTime = 0;
syn.loop = async (currentTime) => {
    ...
    if (!syn.isPaused()) {
        ...
        wall6.render(syn);
        light.updatePosition(Math3.addVec3(player.getPosition(), Math3.gimbal({ x: 0, y: 1, z: 1 }, Math3.subVec3(Math3.Vec3Z, player.getCamera().getRotation()), "yxz")));
